...
def play_training_qtable_games_x(total_games=TRAINING_GAMES, q_tables=None,
...
def play_training_qtable_games_o(total_games=TRAINING_GAMES, q_tables=None,
                          learning_rate=0.4, discount_factor=1.0, epsilon=0.7,
                          x_strategies=None):
    if q_tables is None:
        q_tables = qtables
...
# def update_training_ingame(q_tables, move_history, q_table_player, board,
#                              learning_rate, discount_factor):
#     game_result_reward = get_game_score_value(q_table_player, board)
...
def get_game_score_value(player,board):
    return SCORE_VALUE[board.get_score(player)]
